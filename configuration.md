...
## Политика доступа `XKeenFull` (полное проксирование устройств)

Помимо основной политики `xkeen`, форк поддерживает зарезервированное имя политики `XKeenFull`. Клиенты, назначенные в эту политику в веб-интерфейсе Keenetic, автоматически проксируются **на всех портах**, без правки `port_proxying.lst` или `xkeen.json`.

Как включить:

1. В веб-интерфейсе Keenetic («Пользователи и доступ» → «Политики доступа» → «Добавить политику») создайте политику с именем `XKeenFull`.
2. Привяжите к ней нужные подключения (провайдер, VPN-туннель и т.д.) — обычно те же, что использует политика `xkeen`.
3. На странице «Список устройств» (или в соответствующем разделе) назначьте устройства в политику `XKeenFull`.
4. Перезапустите XKeen командой `xkeen -restart`.

Политики `xkeen` и `XKeenFull` работают одновременно: устройства в `xkeen` проксируются по правилам `port_proxying.lst`/`port_exclude.lst`, а устройства в `XKeenFull` — полностью. Пользовательские политики из `/opt/etc/xkeen/xkeen.json` тоже продолжают работать независимо.

**Внимание:** режим полного проксирования **значительно нагружает CPU роутера** и может снижать скорость соединения. XKeen при старте выводит соответствующее предупреждение в консоль и syslog (`logread -t | grep XKeen`). Используйте полное проксирование только для устройств, которым действительно нужен весь трафик через прокси.

Имя зарезервированной политики задаётся переменной `name_policy_full` в файле `/opt/etc/init.d/S05xkeen` и по умолчанию равно `XKeenFull`; при необходимости его можно переопределить в этом файле.

---

## Контроль файловых дескрипторов

В среде entware Keenetic довольно низкий предел файловых дескрипторов (fd) выделяемых на один процесс - 1024. Превышение этого значения приводит к отказу прокси-клиенту создавать новые коннекты. На процессорах arm64 суммарно на все процессы отведено около 50000 fd, а на mips процессорах около 12000 fd (цифры могут отличаться для разных моделей роутеров). Оригинал XKeen выделяет для процесса xray 1 миллион! дескрипторов и если xray попытается их занять, роутер может просто зависнуть. Форк XKeen версии 1.1.3.6+, в зависимости от процессора, устанавливает лимит 40000 fd для arm64 и 10000 fd для остальных процессоров, а при достижении 90% от этой цифры, перезапускает прокси-клиент. В параметрах `arm64_fd` и `other_fd` файла `/opt/etc/init.d/S24xray` при необходимости можно откорректировать лимиты, установив другие значения. Контроль fd может быть полезен для высоконагруженных роутеров с большим количеством клиентов. Для типичного self-use применения XKeen, данный функционал использовать необязательно, поэтому по умолчанию контроль открытых файловых дескрипторов прокси-клиентом отключен (включить/отключить можно командой `xkeen -fd`). Возможностью контролировать открытые файловые дескрипторы следует пользоваться в крайних ситуациях, когда ничего уже не помогает, так как при этом прокси-клиент будет постоянно перезапускаться для сброса дескрипторов. Рекомендуется вместо этого найти и устранить причину. Это может быть устройство или приложение, открывающее множество подключений.

---

## Перехват DNS и модули фильтрации Keenetic

XKeen при включённом перехвате DNS (`xkeen -dns on`) перенаправляет все DNS-запросы в прокси на уровне `iptables PREROUTING`. Пакеты уходят в прокси-клиент раньше, чем их успевают обработать встроенные модули фильтрации Keenetic — `NextDNS`, `SafeDNS`, `AdGuard DNS` и аналогичные. На практике это проявляется как нестабильная работа интернета у клиентов, для которых включён и перехват DNS XKeen, и фильтрующий модуль ([issue #19](https://github.com/jameszeroX/XKeen/issues/19)).

Варианты решения (выбрать один):

1. **Включить транзит запросов в модуле фильтрации.** В веб-интерфейсе Keenetic откройте настройки используемого модуля (NextDNS/SafeDNS/AdGuard DNS) и поставьте галку `Разрешить транзит запросов`. Модуль продолжит фильтровать DNS, а XKeen — перехватывать и проксировать.
2. **Использовать сервис как обычный DoH/DoT.** Вместо модуля фильтрации настройте нужный DNS-сервер через `Интернет > DNS-серверы > Добавить сервер DNS-over-HTTPS/TLS`. В этом случае сервис воспринимается как обычный DNS и не конфликтует с перехватом.
3. **Отключить перехват DNS в XKeen.** Командой `xkeen -dns off` и настроить DNS-маршруты внутри прокси-клиента вручную (в `xray` через `dns`-объект, в `mihomo` через секцию `dns`). Тогда модуль фильтрации Keenetic работает как обычно.

Те же три варианта актуальны при использовании функции `Routing > DNS-Based Routes` в KeeneticOS 5+, так как причина конфликта общая — DNS-пакеты перехватываются до того, как их видит роутер.
...
## OffLine-установка
Обычная установка XKeen и необходимых компонентов выполняется в OnLine режиме и жёстко привязана к GitHub, а в случае его недоступности будет невозможна. Поэтому в форк дополнительно к способу установки через [Self-Hosted](https://github.com/levmnkv/XKeen/blob/main/configuration.md#self-hosted-прокси-для-загрузки)-прокси добавлен режим OffLine-установки по команде `xkeen -io`
