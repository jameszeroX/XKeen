about_xkeen() {
    echo
    printf "  Утилита ${green}XKeen${reset} предназначена для организации защищённого\n  периметра домашней сети под управлением роутеров ${yellow}Keenetic${reset}.\n"
    printf "  Разработчики ${red}не несут ответственности${reset} за использование\n  ${green}XKeen${reset} вне прямого назначения. Перед использованием убедитесь,\n  что ваши действия соответствуют законодательству вашей страны.\n  Использование ${green}XKeen${reset} в противоправных целях ${red}строго запрещено${reset}.\n"
}
