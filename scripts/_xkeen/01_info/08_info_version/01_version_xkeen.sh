...
info_version_xkeen() {
    version=$(eval curl $curl_extra --connect-timeout 10 $(get_curl_timeout) -s "$xkeen_api_url" | jq -r '.tag_name // .name // ""' 2>/dev/null)

    if [ -z "$version" ]; then
        echo
        printf "${red}Нет доступа${reset} к ${yellow}GitHub API${reset}, пробуем ${yellow}jsDelivr${reset}...\n"
        version=$(eval curl $curl_extra --connect-timeout 10 $(get_curl_timeout) -s "$xkeen_jsd_url" | jq -r '.versions | first' 2>/dev/null)
