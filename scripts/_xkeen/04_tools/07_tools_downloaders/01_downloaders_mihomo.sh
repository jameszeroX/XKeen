...
download_mihomo() {
    ...
    RELEASE_TAGS=$(eval curl $curl_extra --connect-timeout 10 $(get_curl_timeout) -s "${mihomo_api_url}?per_page=20" 2>/dev/null | jq -r '.[] | select(.prerelease == false) | .tag_name' | head -n 8)
    ...
    if [ -z "$RELEASE_TAGS" ]; then
        ...
        RELEASE_TAGS=$(eval curl $curl_extra --connect-timeout 10 $(get_curl_timeout) -s "$mihomo_jsd_url" 2>/dev/null | jq -r '.versions[]' | head -n 8)
