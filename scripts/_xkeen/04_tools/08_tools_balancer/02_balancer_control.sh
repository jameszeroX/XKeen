...
#
# Правило api пишется ПЕРВЫМ прямо в 05_routing.json, а не отдельным файлом:
# Xray при мердже confdir заменяет routing.rules последним файлом, а не
# склеивает — правило из отдельного файла потерялось бы. 05_routing.json
# генерируется XKeen, поэтому при его перегенерации правило нужно вернуть
# (повторным включением) — это известное ограничение.
sb_ensure_api() {
