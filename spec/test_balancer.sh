...
# Известный FAIL: с двухуровневым путём xkeen.xray.speed_balancer
...
# комментариев на промежуточных уровнях пути), а не ошибка схемы .xray
# в самом тесте.
...
# Известный FAIL: sb_ask_measure в ветке "уже включена" не вызывается —
...
# сейчас?" никогда не печатается независимо от .xray-пути.
...
# Известный FAIL: тот же закомментированный sb_ask_measure — ответ "y"
