...
- ~~Релизы XKeen теперь подписываются (.minisig) и проверяются при загрузке~~ (бинарный файл minisign отсутствует в mipsle варианте, ищется другое аналогичное решение для верификации XKeen); ядра xray/mihomo сверяются по sha256 с апстримом напрямую (не через то же зеркало, что отдало файл); режим .xkeen.verify_downloads (по умолчанию warn, не ломает offline-сценарии); починка определения размера у зеркала через Content-Range; отдельный workspace на каждый вызов вместо общего /tmp/xkeen, чтобы параллельные -ug/-um не пересекались [#119](https://github.com/jameszeroX/XKeen/pull/119) - [@dmiales](https://github.com/dmiales)
