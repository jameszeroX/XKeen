...
- DNS-запросы клиентов политик XKeen в журнале AdGuard Home теперь могут отображаться со своими IP-адресами, а не с IP роутера (переменная `-aghfix`)
...
- При включении/отключении контроля файловых дескрипторов теперь не требуется перезагружать роутер, достаточно перезапуска XKeen
