...
- Закрыт обход built-in политики Keenetic «Без доступа в интернет» при включенном проксировании. Реализовано через ipset `xkeen_deny_mac` (hash:mac) с RETURN-правилом в PREROUTING до xkeen-перехвата; синхронизация MAC-ов с `rci/show/ip/hotspot` выполняется на старте XKeen, при каждом netfilter rewrite (`/opt/etc/ndm/netfilter.d/proxy.sh`) и на schedule.d-событиях (`/opt/etc/ndm/schedule.d/00-xkeen-hotspot-sync.sh`) — что даёт динамику для родительского контроля Keenetic без необходимости `xkeen -restart`
- Доработки согласно PR [#32](https://github.com/jameszeroX/XKeen/pull/32) - [@levmnkv](https://github.com/levmnkv)
