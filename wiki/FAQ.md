...
XKeen это набор скриптов для роутеров Keenetic с прокси клиентом Xray-core/Mihomo, с помощью которого можно оптимизировать производительность интернет-подключения и обеспечить его безопасность. Подробнее о проекте - [XKeen на форуме Keenetic](https://forum.keenetic.ru/topic/16899-xkeen/)

FAQ по XKeen предназначен для тех, у кого возникли дополнительные вопросы после внимательного прочтения [инструкции](https://github.com/Corvus-Malus/XKeen/), а также охватывает ситуации, которые напрямую к XKeen не относятся, но возникают в процессе его использования. FAQ в основном посвящен вопросам использования ядра Xray, которым пользуется автор данного материала, разбор вопросов по второму поддерживаемому ядру Mihomo ожидайте от пользователей этого ядра.
...
**A:** При внезапных проблемах с XKeen, причина может быть в устаревшем ядре Xray. Оригинальный XKeen не обновляет ядро выше версии 1.8.4. Если у вас установлен оригинал, то рекомендую перейти на [форк XKeen](https://github.com/jameszeroX/XKeen). При невозможности перехода на форк, обновите ядро Xray в роутере [другим способом](https://github.com/Corvus-Malus/XKeen?tab=readme-ov-file#обновление-ядра-xray-до-последней-версии). Общий порядок действий по поиску причины неработающего проксирования следующий:
...
Если ничего из этого не помогло, выполните `xkeen -diag` и прикрепите диагностический отчёт к сообщению в [телеграм-чате](https://t.me/+8Cvh7oVf6cE0MWRi), описав результаты диагностики по каждому из вышеперечисленных пунктов.

### 1

**Q:** Как правильно обновляться с оригинала XKeen версии 1.1.3 на форк XKeen?
...
Для повышения скорости интернета и времени отклика через прокси рекомендуется выполнить оптимизацию сетевого стека сервера, включив режим [BBR](https://www.google.com/search?&q=как+включить+bbr+на+сервере)
...
**A:** Вы пытаетесь подключиться к прокси, будучи уже подключенными к прокси. Исключите адрес сервера из проксирования, добавив его IP с 32 маской в файл /opt/etc/xkeen/ip_exclude.lst по образцу - [ip_exclude.lst](https://jameszero.net/files/ip_exclude.lst) и перезапустите XKeen. Данный способ работает в XKeen версии 1.1.3.9 и выше.
...
> **Важно!**
>
> Проблема решена в [XKeen версии 1.1.3.9](https://github.com/jameszeroX/XKeen) и новее. Следующия информация оставлена для истории и не является актуальной.

**A:** В некоторых случаях entware со всеми установленными пакетами стартует раньше, чем прошивка завершает инициализацию. В этот момент ещё недоступно Интернет-подключение и не созданы "Политики доступа в интернет", поэтому проксирование запускается для всего устройства или не запускается вовсе. Подберите оптимальную задержку автостарта командой `xkeen -d 30` (где число это секунды, по умолчанию при установке задержка равна 20). Так же можете дополнительно перезапускать XKeen после автостарта, поместив по пути /opt/etc/init.d/ скрипт [S99xkeenrestart](https://jameszero.net/files/S99xkeenrestart.zip) и сделать его исполняемым командой `chmod +x /opt/etc/init.d/S99xkeenrestart`
...
Для XKeen 1.1.3.9 и более старых версий поместите скрипт [aghfix.sh](https://jameszero.net/files/aghfix.zip) в папку /opt/etc/ndm/netfilter.d/ и сделайте его исполняемым командой `chmod +x /opt/etc/ndm/netfilter.d/aghfix.sh` После чего перезагрузите роутер.
...
Форк XKeen начиная с версии 1.1.3.6 умеет [контролировать число отрытых файловых дескрипторов](https://github.com/jameszeroX/XKeen/blob/main/FileDescriptors.md) и перезапускать прокси-клиент при достижении лимита. Чтобы включить эту функцию выполните следующую команду и перезагрузите роутер:
...
- [Отказоустойчивый прокси-сервер xray. Часть 1](https://jameszero.net/4001.htm)
- [Отказоустойчивый прокси-сервер xray. Часть 2](https://jameszero.net/4025.htm)
- [Отказоустойчивый прокси-сервер xray. Часть 3](https://jameszero.net/4576.htm)
