...
**A:** Доступно два способа.

#### Способ 1 (рекомендуемый) — через метку политики XKeen

Работает с failover: провайдера выбирает сама политика, в том числе с резервированием.

1. В веб-интерфейсе Keenetic создайте политику «Приоритеты подключений» с именем `XKeen` и отметьте в ней нужного провайдера.
2. Узнайте метку командой `xkeen -diag` — найдите блок **«Метка политики XKeen для проксирования через провайдера»** и возьмите **десятичное** значение.
3. В конфиге ядра замените `255` на это десятичное значение:
   - **Xray** — поле `mark` в `streamSettings.sockopt` во всех outbound (включая direct)
   - **Mihomo** — параметр `routing-mark` глобально либо в каждом подключении
4. Перезапустите ядро: `xkeen -restart`

XKeen автоматически распознаёт метку и принимает `255`, метку политики `XKeen` и метки **пользовательских политик** из `/opt/etc/xkeen/xkeen.json` (если такая политика создана в роутере с тем же именем). Подробнее — в разделе [«Проксирование трафика Entware»](https://github.com/jameszeroX/XKeen/wiki/Configuration#проксирование-трафика-entware).

Разным outbound можно назначить разные политики (например proxy — через резервного провайдера, direct — через основного). Если нужна привязка не к политике, а напрямую к интерфейсу — Способ 2.

#### Способ 2 — привязка к интерфейсу

Позволяет назначить разные провайдеры на разные outbound. Аналогично пункту 14. В `outbounds.json` добавьте параметр `"interface"` с указанием интерфейса.
