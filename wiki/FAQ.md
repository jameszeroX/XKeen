...
XKeen автоматически распознаёт метку и принимает оба значения (`255` и метку политики).

**Ограничение:** один метод метки = одна политика = один провайдер для всего ядра. Если нужны разные провайдеры для разных outbound — используйте Способ 2.
