...
KeenOS веб-интерфейс → *Интернет* → *Подключения* → *Прокси-сервер* → «Добавить»:

- Адрес: IP роутера в LAN (например `192.168.1.1`)
- Порт: `1080`
- Тип: `SOCKS5`
- Без авторизации

**4. Включите подключение в политике.**

*Сетевые правила* → *Политики доступа в Интернет* → выбрать (или создать) политику для устройства → в списке подключений отметить новый «Прокси-сервер» вместо провайдера. KeenOS сам направит весь трафик устройств этой политики в socks5 → ядро → outbound.
