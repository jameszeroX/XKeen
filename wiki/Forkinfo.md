...
- Поддержка [DSCP-меток QoS](https://jameszero.net/4509.htm) (`61` - force proxy через отдельный inbound/listener, `62` - исключение из проксирования, `63` - проксирование)
...
- Возможность загружать компоненты XKeen через [Self-Hosted прокси](https://github.com/jameszeroX/XKeen/wiki/Configuration#self-hosted-прокси-для-загрузки-с-github) при недоступности GitHub. Пользовательский прокси задаётся в параметре `gh_proxy` конфигурационного файла `xkeen.json`
...
- Для DSCP 61 в режиме Hybrid реализован полноценный split force-path: `TCP -> Redirect`, `UDP -> TProxy`. Старый single `tproxy` force inbound остаётся совместимым для режима TProxy, а в Hybrid теперь диагностируется как неполная конфигурация
