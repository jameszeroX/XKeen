...
### Вариант 2: mark политики XKeen

Если нужно, чтобы исходящие подключения Xray к proxy/VLESS/Reality-серверу ходили через подключения, выбранные в политике доступа `XKeen`, вместо `255` можно использовать mark этой политики.
...
При включённом `xkeen -pr` mark политики `XKeen` также считается служебной bypass-меткой: такие исходящие подключения Xray не будут повторно перехвачены OUTPUT-цепочкой XKeen.

Важно: один outbound не может одновременно использовать `mark: 255` и mark политики `XKeen`. Для каждого outbound выберите один вариант:

- `mark: 255` — только защита от повторного перехвата при проксировании Entware;
- `mark: <decimal mark политики XKeen>` — защита от повторного перехвата плюс маршрутизация outbound Xray по таблице политики `XKeen`.
