...
XKeen не прописывает эти marks автоматически в конфиги Xray/Mihomo — это ручной сценарий: получите mark нужной политики и добавьте его в конфиг прокси-клиента. После настройки включите проксирование командой `xkeen -pr`.

### Вариант 1: стандартная служебная метка 255

Защищает исходящие подключения прокси-клиента от повторного перехвата при включённом `xkeen -pr`. Xray — добавьте к каждому outbound, включая `direct`:
```
"streamSettings": {
  "sockopt": {
    "mark": 255
...
```
Mihomo — глобально (либо к каждому `proxies` / `proxy-providers` / `direct`):
```
tproxy-port: 1181
log-level: silent
...
...
### Вариант 2: mark политики XKeen или пользовательской политики (multi-WAN)

При нескольких провайдерах метка `255` уходит через основное подключение независимо от политики. Чтобы исходящие подключения ядра шли через провайдера, выбранного в политике доступа `XKeen` (или в пользовательской политике из `/opt/etc/xkeen/xkeen.json`, созданной в роутере с тем же именем) — вместо `255` укажите **десятичный** mark этой политики. Решение работает с failover: провайдера выбирает сама политика.

Проще всего узнать mark политики `XKeen` командой `xkeen -diag` — блок **«Метка политики XKeen для проксирования через провайдера»** (показывает hex и decimal). Вручную:
```
# политика XKeen
...
MARK_HEX=$(curl -kfsS localhost:79/rci/show/ip/policy | jq -r --arg p "$POLICY_NAME" '.[] | select((.description // "" | ascii_downcase) == ($p | ascii_downcase)) | .mark')
printf "%d\n" "0x$MARK_HEX"
```
Полученное **десятичное** значение впишите в Xray `streamSettings.sockopt.mark` (в каждый нужный outbound) либо в Mihomo `routing-mark` (глобально / в `proxies` / `proxy-providers`):
```
"sockopt": { "mark": 268434093 }      # Xray; 268434093 — только пример
routing-mark: 268434093               # Mihomo; используйте значение со своего роутера
```
При включённом `xkeen -pr` mark политики `XKeen` и marks пользовательских политик считаются служебными bypass-метками: такие исходящие подключения ядра не перехватываются повторно цепочкой OUTPUT XKeen, а Keenetic маршрутизирует их по таблице выбранной политики.

> Один outbound несёт ровно одну метку: либо `255` (только защита от петли, выход через основное подключение), либо decimal mark политики (защита от петли + маршрутизация через выбранного провайдера). Разным outbound можно назначить разные политики. XKeen принимает оба варианта; для single-WAN поведение не меняется.

Для привязки конкретного outbound к интерфейсу напрямую (без политики) см. [FAQ #22](https://github.com/jameszeroX/XKeen/wiki/FAQ#22-два-провайдера--подключение-через-один).
