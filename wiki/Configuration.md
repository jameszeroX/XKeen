...
Фича необязательная и по умолчанию выключена. Включается командой `xkeen -sb on` (при первом включении предложит автоматически добавить блок `api` и probe-inbound в конфигурацию Xray) или правкой `xkeen.json`. Включать нужно на **запущенном** Xray: у остановленного ядра api молчит независимо от того, настроен он или нет, поэтому на остановленном XKeen команда только попросит выполнить `xkeen -start`, а не предложит добавить api и probe второй раз.
