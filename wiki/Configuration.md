...
### Выбор провайдера (multi-WAN)

При наличии двух и более провайдеров трафик ядра по умолчанию уходит через основное подключение независимо от настроек политики. Чтобы egress ядра шёл через конкретного провайдера (включая резервного):

1. В веб-интерфейсе Keenetic создайте политику «Приоритеты подключений» с именем `XKeen` и отметьте в ней нужного провайдера.
2. Узнайте метку политики командой `xkeen -diag` — найдите блок **«Метка политики XKeen для проксирования через провайдера»**, он покажет метку в hex и в десятичном виде. В конфиг ядра вписывается **десятичное** значение.
3. Замените `255` на это десятичное значение:
   - **Xray** — в каждом outbound (включая direct), поле `streamSettings.sockopt.mark`
   - **Mihomo** — в параметре `routing-mark` глобально, либо в каждом подключении

XKeen автоматически распознаёт метку и принимает как значение `255`, так и метку политики — регрессий для single-WAN нет.

**Ограничение:** один метод метки = одна политика = один провайдер для всего ядра. Если нужны разные провайдеры для разных outbound — используйте привязку к интерфейсу через `sockopt.interface` (см. [FAQ #22](https://github.com/jameszeroX/XKeen/wiki/FAQ#22-два-провайдера--подключение-через-один)).
