...
- если включён `multiwan`, XKeen не даст запуститься с неверным конфигом.
...
- пропишите `255`;
- используйте `xkeen -multiwan off`.
...
- `xkeen -multiwan off` — обычный режим, всё идёт как раньше через `default`;
- `xkeen -multiwan on` — Xray/Mihomo должны идти через политику `XKeen` и выбранные для неё соединения;
- `xkeen -multiwan status` — показывает текущий режим.
...
Если `mark` отсутствует, неверный или конфиг нельзя нормально проверить, XKeen не даст включить `multiwan`.
