...
## Проксирование через выбранного провайдера

Данный режим даёт возможность пустить проксирование через одного из провайдеров интернета при наличии нескольких

Подробный вариант настройки PBR (Policy-Based Routing) рассмотрен в [следующем пункте](https://github.com/jameszeroX/XKeen/wiki/Configuration#проксирование-через-выбранного-провайдера)

Основные моменты:

- необходима маркировка исходящих подключений прокси-клиента согласно `mark` используемой политики XKeen
- узнать значение `mark` можно по команде `xkeen -pbr codes`
- после добавления `mark` в конфигурационный файл прокси-клиента, включить режим работы через выбранного провайдера можно командой `xkeen -pbr on` (отключить `xkeen -pbr off`)
...
Выберите необходимого провайдера в приоритетах подключения политики и перезапустите XKeen (`xkeen -restart`)
