...
- `61` - принудительная отправка трафика в отдельный TProxy inbound Xray
...
## DSCP 61: принудительное проксирование через отдельный inbound Xray

Метка `61` не отправляет трафик напрямую в outbound Xray. Вместо этого XKeen перехватывает такой трафик и отправляет его в отдельный transparent TProxy inbound. Дальше Xray сам выбирает нужный outbound по `inboundTag`.
