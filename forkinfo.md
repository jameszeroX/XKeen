...
- Поддержка ядра Mihomo и смена ядра проксирования (Xray/Mihomo) параметрами `-xray` и `-mihomo`
...
- Реализована работа с IPSET и возможность исключать из проксирования IP-подсети России (параметры `-gips`, `-dgips`) [подробнее](https://github.com/jameszeroX/XKeen/blob/main/configuration.md#описание-работы-ipset-в-xkeen)
- Поддержка [DSCP-меток QoS](https://jameszero.net/4509.htm) (`62` - исключение из проксирования, `63` - проксирование)
- Возможность проксирования трафика Entware (параметр `-pr`) [подробнее](https://github.com/jameszeroX/XKeen/blob/main/configuration.md#проксирование-трафика-entware)
...
- Параметр запуска `-di` - Время ожидания инициализации роутера перед началом запуска прокси-клиента
- Параметр запуска `-xtest` - Проверить конфигурацию Xray на ошибки
- Параметр `-toff` для отключения таймаута загрузок при замедлении GitHub. Пример использования: `xkeen -i -toff`
- Возможность резервного копирования и восстановления конфигурации Mihomo (параметры запуска `-mb`, `-mbr`)
- Возможность контролировать число открытых файловых дескрипторов, используемых прокси-клиентом и перезапускать процесс при исчерпании лимита (параметр запуска `-fd`) [подробнее](https://github.com/jameszeroX/XKeen/blob/main/configuration.md#контроль-файловых-дескрипторов)
- Предусмотрен вывод стандартной либо расширенной информации при запуске прокси-клиента (параметр запуска `-extmsg`)
...
- При запуске или перезапуске XKeen теперь отображается информация о режиме работы - TProxy, Hybrid (ранее Mixed), Redirect, Other
...
- Скрипт запуска S24xray переименован в S99xkeen, затем в XKeen 2.0 — в S05xkeen (для совместимости с IPSET-компонентом)
...
- Доработки согласно PR [#32](https://github.com/jameszeroX/XKeen/pull/32), [#57](https://github.com/jameszeroX/XKeen/pull/57), [#58](https://github.com/jameszeroX/XKeen/pull/58), [#59](https://github.com/jameszeroX/XKeen/pull/59) - [@kittylabassistant](https://github.com/kittylabassistant)
...
- Неиспользуемые параметры запуска `-x`, `-rk`, `-rx`, `-rc` `-rrk`, `-rrx`, `-rrm`, `-drk`, `-drx`, `-drm`