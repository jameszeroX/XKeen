...
- При включении перехвата DNS (`xkeen -dns on`) выводится предупреждение о несовместимости с функцией DNS-маршрутизации KeeneticOS 5+ (Routing → DNS-Based Routes): при активном перехвате DNS-пакеты перехватываются на уровне iptables PREROUTING раньше, чем их видит роутер, из-за чего DNS-маршруты Keenetic не работают
