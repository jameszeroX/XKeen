...
| `-multiwan` | Режим выхода Xray/Mihomo в интернет: `off` — как обычно через `default`, `on` — через выбранную политику/WAN, `status` — показать текущий режим, `codes` — вывести коды политики `XKeen` и пользовательских политик для копирования |
