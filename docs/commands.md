...
| `-pbr` | PBR-проверка для Xray/Mihomo: `off` — обычный режим через `default`, `on` — строгая проверка корректного `mark` / `routing-mark` выбранной политики Keenetic, `status` — показать текущий режим, `codes` — вывести коды политики `XKeen` и пользовательских политик для копирования |
